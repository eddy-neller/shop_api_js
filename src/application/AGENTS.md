...
- **Valeurs applicatives immuables** : une classe Application comme `Pagination` peut regrouper une normalisation pure utile a plusieurs use cases. Ce n'est pas un VO Domain : elle ne porte aucun invariant metier, n'est jamais transportee dans une Command/Query ni construite en Presentation. Le use case la construit a partir des primitives, puis transmet les scalaires normalises au port. Voir `docs/architecture/application/shared/pagination.md`.
